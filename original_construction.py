    ...
    while index < len(self.vertices_by_degree):
        ...
        if current_seed in considered_vertices:
            ...
            while improvement_flag and (add_candidates or remove_candidates):
                ...
                ############################################################
                best_add_change, best_add_change_score = find_best_add(self, add_candidates, current_cluster, current_score, current_cluster_weight_in, current_cluster_weight_out)

                ############################################################
                # CONSIDER REMOVING A VERTEX ON THE BOUNDARY
                ############################################################
                best_remove_change, best_remove_change_score = find_best_remove(self, remove_candidates, current_cluster, current_cluster_weight_in, current_cluster_weight_out, current_score)
                ...
                if should_add:
                    ...
                    current_cluster_weight_in, current_cluster_weight_out = add(self, add_candidates, current_cluster, remove_candidates, best_add_change, best_add_change_score, current_cluster_weight_in, current_cluster_weight_out)
                elif should_remove:
                    improvement_flag = True
                    current_score = best_remove_change_score
                    current_cluster_weight_in, current_cluster_weight_out = remove(self, remove_candidates, add_candidates, current_cluster, best_remove_change, best_remove_change_score, current_cluster_weight_in, current_cluster_weight_out)
